...
// ─── Post-Trade Analysis ──────────────────────────────────────────────────
export function useGeneratePostTradeAnalysis() {
  const { actor } = useActor(createActor);
  const [isLoading, setIsLoading] = useState(false);
...
// ─── Price Prediction ─────────────────────────────────────────────────────
export function useGeneratePricePrediction() {
  const { actor } = useActor(createActor);
  const [isLoading, setIsLoading] = useState(false);

  const generate = useCallback(
    async (asset: string, sessionToken: string): Promise<string | null> => {
      setIsLoading(true);
      try {
        if (!actor) return null;
        const result = await actor.generatePricePrediction(asset, sessionToken);
        if (result.__kind__ === "ok") return result.ok;
        return null;
      } catch {
        return null;
...
export function useCompareSignals() {
  ...
  const compare = useCallback(
    async (
      ...
      try {
        if (!actor) return null;
        const result = await actor.compareSignals(asset1, asset2, sessionToken);
        if (result.__kind__ === "ok") return result.ok;
        return null;
      } catch {
        return null;
...
// ─── Signal Chain ─────────────────────────────────────────────────────────
export function useGenerateSignalChain() {
  const { actor } = useActor(createActor);
  const [isLoading, setIsLoading] = useState(false);
  ...
  const generate = useCallback(
    async (asset: string, sessionToken: string): Promise<string | null> => {
      ...
      try {
        if (!actor) return null;
        const result = await actor.generateSignalChain(asset, sessionToken);
        if (result.__kind__ === "ok") return result.ok;
        return null;
      } catch {
        return null;
...
// ─── Analyze News Impact ──────────────────────────────────────────────────
export function useAnalyzeNewsImpact() {
  const { actor } = useActor(createActor);
  const [isLoading, setIsLoading] = useState(false);

  const analyze = useCallback(
    async (headline: string, sessionToken: string): Promise<string | null> => {
      setIsLoading(true);
      try {
        if (!actor) return null;
        const result = await actor.analyzeNewsImpact(headline, sessionToken);
        if (result.__kind__ === "ok") return result.ok;
        return null;
      } catch {
        return null;
...
// ─── AI-Powered Token FAQ ─────────────────────────────────────────────────
export function useAskTokenFaq() {
  const { actor } = useActor(createActor);
  const [isLoading, setIsLoading] = useState(false);
  ...
  const ask = useCallback(
    async (question: string): Promise<string | null> => {
      ...
      try {
        if (!actor) return null;
        const answer = await actor.askTokenFaq(question);
        return answer || null;
      } catch {
        return null;
      } finally {
